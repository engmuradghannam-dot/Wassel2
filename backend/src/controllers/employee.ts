...
export const updateEmployee = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (existing) {
      ...
      const housing = Number(data.housingAllowance ?? existing.housingAllowance);
      const transport = Number(data.transportAllowance ?? existing.transportAllowance);
      const other = Number(data.otherAllowance ?? existing.otherAllowance);
