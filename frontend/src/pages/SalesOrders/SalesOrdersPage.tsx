...
export const SalesOrdersPage = () => {
  ...
  const { salesOrders, isLoading, createSalesOrder, submitSalesOrder, cancelSalesOrder, isSaving } = useSalesOrders();
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (validLines.length === 0 || !customerId) return;
  ...
  const handleSubmitOrder = async (order: SalesOrder) => {
    if (confirm(`اعتماد أمر الشراء ${order.orderNumber}؟`)) await submitSalesOrder(order.id);
  };
  const handleCancel = async (order: SalesOrder) => {
    if (confirm(`إلغاء أمر الشراء ${order.orderNumber}؟`)) await cancelSalesOrder(order.id);
  };
  ...
  return (
    <div className="space-y-4">
          ...
          <tbody className="table-body">
            ...
            {salesOrders.map((order) => (
              <tr key={order.id}>
                ...
                <td className="table-cell">{(order as any).customer?.name || '-'}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="أمر بيع جديد">
                ...
                <div key={i} className="grid grid-cols-12 gap-2 items-center">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateLine(i, { itemId: e.target.value, unitPrice: item ? item.sellingPrice : 0 });
                    }}
                  >
